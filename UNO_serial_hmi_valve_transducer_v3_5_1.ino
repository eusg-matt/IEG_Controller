...
//  11 Vacuum tank interlock is not made.                                      ++NOT IMPLEMENTED - FUTURE REVISION++
...
// test that fill and single shot request are executed when in pump down process.
...
//Provide a way for users to change the pump and fill targets, and timeouts through the IOC.
...
//-----------------------------------------Pump, Purge & Fill IVC----------------------------------------------------------------------------------------
  if(HMICmd==1 || SerialCmd==1){
    ...
    for(int i=0; i<purge_cycles; i++){
      if(PumpDownSampleVol()){               //PUMP Function
        if(PurgeSampleVolFromSource()){      //PURGE Function
          purge_failed=false;
        }
        else{
    ...
    if(!purge_failed){   
      if(PumpDownSampleVol()){                                      //PRE-FILL PUMP Function
        if(FillSampleVolFromBuffer(fill_pressure)){                 //FILL Function
...
//----------------------------------Hot Stick - Furnace Mode - Continous Pump--------------------------------------------------------------------------------------------
     
  else if(HMICmd==2 || SerialCmd==2){

     ErrorStatus=0;
     OpMode=2;
     Pump_LEDSet();
     while(PumpDownSampleVol());     //stay looping in this function until the function returns false; i.e. process error or abort command
     ClearButtonLEDs();
  }
     
//---------------------------------Sample Change - Continous Gas Flow ------------------------------------------------------------------------------------------------------
...
//---------------------------------Single Shot - Exchange gas from buffer volume -------------------------------------------------------------------------------------------
...
//    Serial1.println("check for abort2");
  ...
  //++++________________________________________________new code to test_____________________________________________________________++++

    if(serial_abort_check==3 || PushButton_State()==3){   //Abort pump if recieved a fill request
      SetValves_Default();                                //Close all valves (stop pumping volume)
      return false;                                       //Return from function
    }

    if(serial_abort_check==4 || PushButton_State()==4){   //Abort pump if recieved a single shot request
      SetValves_Default();                                //Close all valves (stop pumping volume)
      return false;                                       //Return from function
    }
